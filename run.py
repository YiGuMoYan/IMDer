...
from trains.utils import setup_seed
...
def IMDER_run(
    ...
    batch_data = dataloader['train'].dataset[0]
    vision = batch_data['vision'].unsqueeze(0).to(args.device)
    audio = batch_data['audio'].unsqueeze(0).to(args.device)
    text = batch_data['text'].unsqueeze(0).to(args.device)
    ...
    flops, params = profile(model, (text, audio, vision, 1))
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    # print('pt/imder-{}.pth'.format(args.dataset_name))
