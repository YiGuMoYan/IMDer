...
class IMDER():
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        while True:
            ...
            # 绘制loss
            writer.add_scalar('Training Loss', round(train_loss, 4), e * len(dataloader['train']))
            writer.add_scalar('Training Acc_2', round(train_results['Acc_2'], 4), e * len(dataloader['train']))
            writer.add_scalar('Training Acc_7', round(train_results['Acc_7'], 4), e * len(dataloader['train']))
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False):
        ...
        # 绘制loss
        writer.add_scalar(f'{mode} Loss', round(eval_results['Loss'], 4), e * len(dataloader))
        writer.add_scalar(f'{mode} Acc_2', round(eval_results['Acc_2'], 4), e * len(dataloader))
        writer.add_scalar(f'{mode} Acc_7', round(eval_results['Acc_7'], 4), e * len(dataloader))
